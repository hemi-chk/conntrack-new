...
export const AddVehicleModal = ({ isOpen, onClose, onAdd }) => {
  ...
  // Calculate minimum date for the insurance expiry calendar (blocks past dates)
  ...
  const handleAdd = () => {
    ...
    if (!formData.vehicle_number || !formData.type || !formData.availability_status || !formData.insurance_expiry || !formData.port_pass_expiry) {
        setError("Please fill out all required details before adding the vehicle.");
        return;
    }
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      {/* Modal Container: Flex layout to split Form (left) and Summary (right) */}
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col md:flex-row overflow-hidden border border-gray-100 animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex-1 p-8">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-500 mt-1">Enter the details of the new vehicle</p>
          </div>
            ...
            <div className="grid grid-cols-2 gap-5">
              <div>
                ...
                <input 
                  ...
                  min={today} // Prevents past dates via HTML5 calendar native logic
        ...
        <div className="w-full md:w-[320px] bg-surface-light border-t md:border-t-0 md:border-l border-gray-200 p-8 flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-5">
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider font-semibold mb-1">Vehicle No:</p>
                <p className={`font-medium ${formData.vehicle_number ? 'text-dark' : 'text-gray-400'}`}>
                  {formData.vehicle_number || 'Not set'}
                </p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider font-semibold mb-1">Type:</p>
                <p className="font-medium text-dark capitalize">{formData.type}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider font-semibold mb-1">Insurance Exp:</p>
                <p className={`font-medium ${formData.insurance_expiry ? 'text-dark' : 'text-gray-400'}`}>
                  {formData.insurance_expiry ? new Date(formData.insurance_expiry).toLocaleDateString('en-GB') : 'Not set'}
                </p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider font-semibold mb-1">Port Pass Exp:</p>
                <p className={`font-medium ${formData.port_pass_expiry ? 'text-dark' : 'text-gray-400'}`}>
                  {formData.port_pass_expiry ? new Date(formData.port_pass_expiry).toLocaleDateString('en-GB') : 'Not set'}
                </p>
              </div>
            </div>
          </div>
