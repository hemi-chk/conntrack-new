...
function BiddingOrdersTable({
  ...
  const [awardStatusFilter, setAwardStatusFilter] =
    useState("All");
  const [currentPage, setCurrentPage] = useState(1);

  const showBidsColumn =
    tableMode !== "created";

  const showOrderStatusColumn =
    tableMode === "result";
  ...
  const formatOrderType = (value) => {
    ...
    return (
      valueText.charAt(0).toUpperCase() +
      valueText.slice(1)
    );
  };
  ...
  const getAwardDisplayState = (order) => {
    const award =
      getAwardStateForOrder?.(order);

    const workflowState =
      normalizeStatus(
        award?.awardWorkflowState
      );
    ...
    tableMode === "result"
      ? (() => {
          ...
          const orderedKnownStatuses =
            AWARD_STATUS_PROCESS_ORDER.filter(
              (status) =>
                presentStatuses.includes(
                  status
                )
            );
          ...
          const additionalStatuses =
            presentStatuses
              .filter(
                (status) =>
                  !AWARD_STATUS_PROCESS_ORDER.includes(
                    status
                  )
              )
              .sort((a, b) =>
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
              ...
              {tableMode ===
                ...
                <select
                  ...
                  onChange={(e) =>
                    setAwardStatusFilter(
                      e.target.value
                    )
                  }
                  className="min-w-[260px] rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium text-[#1E293B] outline-none focus:border-[#5483B3] focus:ring-2 focus:ring-[#EBF4FF]"
                >
                  <option value="All">
                    All Award Statuses
                  </option>

                  {awardStatusOptions.map(
                    (status) => (
                      <option
                        key={status}
                        value={status}
                        ...
                        >
                          ...
                          {tableMode ===
                            ...
                            <td className="px-4 py-3 border-t border-slate-100 whitespace-nowrap">
                              ...
                              <span
                                className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${awardDisplay.className}`}
                              >
                                {
                                  awardDisplay.label
                                }
