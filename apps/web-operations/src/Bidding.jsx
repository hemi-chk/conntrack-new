...
function Bidding() {
  ...
  // Maximum 5 bids are sent to Logistics.
  // If fewer than 5 bids exist, all available bids must be selected.
  const requiredShortlistCount = useMemo(
  ...
  const toggleShortlist = (
    ...
    setShortlistedBidIds(
      (prev) => {
        ...
        if (
          prev.length >=
          requiredShortlistCount
        ) {
          alert(
            requiredShortlistCount < 5
              ? `Only ${requiredShortlistCount} bid${requiredShortlistCount === 1 ? " is" : "s are"} available. Please shortlist all available suppliers.`
  ...
  const sendShortlistedToLogistics = async () => {
    ...
    if (
      ...
      alert(
        requiredShortlistCount < 5
          ? `Only ${requiredShortlistCount} bid${
              requiredShortlistCount === 1 ? " is" : "s are"
            } available. Please shortlist all ${requiredShortlistCount} before sending to Logistics.`
          : "Please shortlist exactly 5 suppliers before sending to Logistics."
      );
    ...
    if (
      ...
      alert(
        `Selected bids are not valid for this order. Please select the required ${requiredShortlistCount} bid${
    ...
    try {
      ...
      alert(
        `${requiredShortlistCount} shortlisted supplier${
          requiredShortlistCount === 1 ? "" : "s"
  ...
  const getNotificationStatus = (bid) => {
    ...
    if (sentToLogistics && freshWinner) {
      return "Ready to Notify";
    }
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {sentToLogistics
                ...
                : `Send Shortlisted to Logistics (${shortlistedBidIds.length}/${requiredShortlistCount})`}
