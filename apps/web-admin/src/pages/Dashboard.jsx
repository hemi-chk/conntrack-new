...
function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Hero Banner */}
      {/* WHY: Personalized greeting makes the system feel alive */}
      <div className="relative bg-gradient-to-r from-[#1E40AF] to-blue-400 rounded-2xl p-8 overflow-hidden shadow-lg">
        {/* Background decoration */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-white opacity-5 rounded-full -translate-y-1/2 translate-x-1/2" />
        <div className="absolute bottom-0 right-32 w-32 h-32 bg-white opacity-5 rounded-full translate-y-1/2" />
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div>
            ...
            <button className="mt-4 flex items-center gap-2 bg-white text-[#1E40AF] px-4 py-2 rounded-xl text-sm font-semibold hover:bg-blue-50 transition shadow-sm">
              Review Issues
              <ArrowRight size={16} />
            </button>
          </div>

          {/* WHY: Stats summary in hero for instant overview */}
          <div className="hidden lg:flex items-center gap-4">
            <div className="bg-white bg-opacity-95 rounded-2xl p-4 text-center min-w-24 shadow-lg">
              <p className="text-3xl font-bold text-white">{loading ? '...' : stats.total_orders}</p>
              <p className="text-blue-200 text-xs mt-1">Total Orders</p>
            </div>
            <div className="bg-white bg-opacity-95 rounded-2xl p-4 text-center min-w-24 shadow-lg">
              <p className="text-3xl font-bold text-white">{loading ? '...' : stats.active_orders}</p>
              <p className="text-blue-200 text-xs mt-1">Active Now</p>
            </div>
            <div className="bg-white bg-opacity-95 rounded-2xl p-4 text-center min-w-24 shadow-lg">
              <p className="text-3xl font-bold text-white">{issues.filter(i => i.type === 'error').length}</p>
              <p className="text-blue-200 text-xs mt-1">Critical Issues</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {statCards.map((stat) => (
          <div key={stat.label} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:shadow-md transition">
      ...
      <div className="grid grid-cols-3 gap-6">
          ...
          <div className="divide-y divide-slate-50">
            {issues.map((issue) => (
              <div key={issue.id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition group">
                ...
                <button className="text-xs font-medium text-[#1E40AF] opacity-0 group-hover:opacity-100 transition flex items-center gap-1 whitespace-nowrap ml-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100">
          <div className="p-5 border-b border-slate-100 flex items-center justify-between">
            ...
            <button className="text-xs text-[#1E40AF] font-medium hover:underline">View all</button>
          </div>
          <div className="divide-y divide-slate-50">
            {recentOrders.map((order) => (
              <div key={order.id} className="p-4 hover:bg-slate-50 transition cursor-pointer group">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-bold text-[#1E40AF]">{order.id}</span>
