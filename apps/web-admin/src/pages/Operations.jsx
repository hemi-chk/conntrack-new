...
function Operations() {
  ...
  useEffect(() => {
    const fetchStaff = async () => {
      try {
        setLoading(true)
        const data = await adminAPI.getStaff('operations')
        setStaff(data || [])
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      await adminAPI.addStaff({ ...formData, role: 'operations', status: 'active', temp_only: true })
      setStaff(await adminAPI.getStaff('operations'))
      setShowForm(false)
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#1E293B]">Operations Team</h1>
          <p className="text-sm text-slate-500 mt-1">Manage operational staff members</p>
        </div>
        <button
              ...
              {filteredStaff.length > 0 ? (
                ...
                <tr>
                  <td colSpan="5" className="py-8 text-center text-slate-400">
                    {searchTerm ? `No staff found matching "${searchTerm}"` : 'No operations staff found'}
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-slate-100 sticky top-0 bg-white rounded-t-2xl">
              <h2 className="text-lg font-bold text-[#1E293B]">Add Operations Staff</h2>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <select name="position" value={formData.position} onChange={handleChange} className="w-full mt-1 px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                      ...
                      <option value="Senior Operations Officer">Senior Operations Officer</option>
